...
const apiKey = "YOUR API KEY FROM OPENWEATHER OR OTHER WEATHER API PLATFORM"
...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput =  
            `<speak>
                <voice name="Matthew">
                Hello!! <break strength = "strong"/> This is Faiq's first Alexa Skill 
                called Weather Assistant
                <break strength = "strong"/> 
                Try Asking Weather from me
                </voice>
            </speak>`;
         ...
         const rePrompt = 
            `<speak>
              <voice name="Matthew">
              I don't get it, <break strength = "strong"/>
              Try Asking Weather from me
              </voice>
            </speak>`;
...
const WeatherIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const city = slots.city.value || "karachi";
        ...
        return await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}`)
              .then((response)=>{
                 ...
                 const speakOutput = 
                        `<speak> 
                            <voice name="Matthew">
                                In ${weatherData.name} <break strength="weak"/> its ${weatherData.weather[0].description} with ${weatherData.main.temp} degree centigrade. 
